    ...
    **decode_options,
    ...
    def decode_with_fallback(segment: torch.Tensor) -> DecodingResult:
        ...
        for t in temperatures:
            ...
            decode_result, logits = model.decode(segment, options)
    ...
    with tqdm.tqdm(
        ...
        while clip_idx < len(seek_clips):
            ...
            result, logits = decode_with_fallback(mel_segment)
            ...
            if len(consecutive) > 0:
                ...
                for current_slice in slices:
                    ...
                    sliced_logits = logits[last_slice:current_slice]
                    segment_logits.append(sliced_logits)
            ...
            all_segments.extend(
                [
                    {"id": i, "logits": seg_logits, **segment}
                    for i, (segment, seg_logits) in enumerate(
                        zip(current_segments, segment_logits), start=len(all_segments)
                    )
                ]
            )
            all_tokens.extend(
